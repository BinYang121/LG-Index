...
void Utilitycfql::GetKCore(const Graphcfql *graph, int *core_table) {
...
unsigned long Utilitycfql::TimeDiffInMicroseconds(const timeval start, const timeval end)
{
    unsigned long micro_second_diff = 1000000 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec;
    return micro_second_diff;
}

double Utilitycfql::TimeDiffInSeconds(const timeval start, const timeval end)
{
    double second_diff = (end.tv_sec - start.tv_sec) + (end.tv_usec - start.tv_usec) / (double)1000000.0;
    return second_diff;
}

unsigned long Utilitycfql::UpperPowerOfTwo(unsigned long v)
{
    v--;
    v |= v >> 1;
    v |= v >> 2;
...
void Utilitycfql::old_cheap(int* col_ptrs, int* col_ids, int* match, int* row_match, int n, int m) {
    int ptr;
    int i = 0;
    for(; i < n; i++) {
...
void Utilitycfql::match_bfs(int* col_ptrs, int* col_ids, int* match, int* row_match, int* visited,
