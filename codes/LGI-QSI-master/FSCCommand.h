...
enum OptionKeyword {
	...
	// Bin Yang, 20b903067@stu.hit.edu.cn, July 2021.
	OutputFile = 5
};
...
private:
    void ProcessOptions() {
		...
		// Bin Yang, 20b903067@stu.hit.edu.cn, July 2021.
        value = GetCommandOption(options_key[OptionKeyword::OutputFile]);
        if (value == "") {
            printf("The output file path can not be empty.\n");
            exit(-1);
        }
        options_value[OptionKeyword::OutputFile] = value;
...
public:
	...
	const string& GetOutputFilePath() {
        return options_value[OptionKeyword::OutputFile];
    }
};
