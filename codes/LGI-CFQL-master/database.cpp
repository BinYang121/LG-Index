...
void Database::readTree ( GraphInforPtr query ) {
  ...
  for ( int i = 0; i < query->numv; i++ ) {
    inputnodelabel = query->qnodelabels[i];
  ...
  for ( int i = 0; i < query->nume; i++ ) {
    nodeid1 = query->qfromnodes[i];
    nodeid2 = query->qtonodes[i];
    inputedgelabel = query->qedgelabels[i];
