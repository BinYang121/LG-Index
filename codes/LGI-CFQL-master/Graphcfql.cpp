...
void Graphcfql::BuildReverseLabelIndex(const int label_num) {
...
void Graphcfql::ComputeLabelFrequency() {
...
void Graphcfql::SortEdge() {
    for (int i = 0; i < vertices_num_; ++i) {
        sort(edges_ + edges_offset_[i], edges_ + edges_offset_[i + 1]);
    }
}

void Graphcfql::SortEdgeByLabel() {
...
void Graphcfql::ComputeMaxDegree() {
    max_degree_ = -1;
    for (int i = 0; i < vertices_num_; ++i) {
...
void Graphcfql::BuildPrunedGraph() {
...
void Graphcfql::BuildDegreeOneVertices() {
