...
void GraphState::print () {
  ...
  counter++;
  QFeatureInforPtr feature = new QFeatureInfor ( counter );
  features.push_back ( feature );
  
  feature->numv = nodes.size ();
  for ( int i = 0; i < nodes.size (); i++ ) {
    feature->nodes.push_back ( i );
    feature->nodelabels.push_back ( database.nodelabels[nodes[i].label].inputlabel );
  }
  ...
  for ( int i = 0; i < nodes.size (); i++ ) {
    for ( int j = 0; j < nodes[i].edges.size (); j++ ) {
      ...
      if ( i < edge.tonode ) {
        feature->fromnodes.push_back ( i );
        feature->tonodes.push_back ( edge.tonode );
		int fedgelabel = database.edgelabels[database.edgelabelsindexes[edge.edgelabel]].inputedgelabel;
		feature->edgelabels.push_back ( fedgelabel );
		nume++;
        }      
    }
  }
...
/*   if ( structure == "path" ) {
		feature->structure = 0;
	} else if ( structure == "path_cycle" ) {
		feature->structure = 1;
	} else if ( structure == "tree" ) {
		feature->structure = 2;
	} else if ( structure == "cycle_graph" ) {
		feature->structure = 3;
	}  */
}
...
void GraphState::clear () {
	...
	features.clear ();
/* 	for ( int i = 0; i < features_qsi.size (); i++ ) {
        delete features_qsi[i];
	}
	features_qsi.clear (); */
