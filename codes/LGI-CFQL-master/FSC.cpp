...
FSC::FSC(GraphDB* graphDB, vector<Graphcfql*>* data_graphs_i) {
	graphDB_ = graphDB;
	InitializeDataGraphResource(data_graphs_i);
}
...
vector<int> FSC::Query(const Graphcfql *query_graph, vector<int> query_results, int &results_count ) {
    ...
    for (int i = 0; i < (int)data_graphs_->size(); ++i) {
        ...
        const Graphcfql *data_graph = data_graphs_->at(i);

        //timeval simulation_start = Utilitycfql::GetTime();

        bool is_valid = Simulation(query_graph, data_graph);

        //timeval simulation_end = Utilitycfql::GetTime();
        //double temp_filtering_time = Utilitycfql::TimeDiffInMicroseconds(simulation_start, simulation_end);
        //filtering_time_ += temp_filtering_time;

        if (is_valid) {
            candidate_graph_num_ += 1;

            //timeval verification_start = Utilitycfql::GetTime();

            is_valid = Verification(query_graph, data_graph);

            //timeval verification_end = Utilitycfql::GetTime();
            //double temp_verification_time = Utilitycfql::TimeDiffInMicroseconds(verification_start, verification_end);
...
void FSC::InitializeDataGraphResource(vector<Graphcfql*>* data_graphs_i) {
	data_graphs_ = data_graphs_i;
    ...
    max_degree_ = -1;
    for (int i = 0; i < (int)data_graphs_i->size(); ++i) {
        if ((*data_graphs_i)[i]->MaxVerticesNumWithSameLabel() > max_data_vertices_with_same_label_) {
            max_data_vertices_with_same_label_ = (*data_graphs_i)[i]->MaxVerticesNumWithSameLabel();
        }

        if ((*data_graphs_i)[i]->VerticesCount() > max_data_vertices_num_) {
            max_data_vertices_num_ = (*data_graphs_i)[i]->VerticesCount();
        }

        if ((*data_graphs_i)[i]->EdgesCount() > max_data_edges_num_) {
            max_data_edges_num_ = (*data_graphs_i)[i]->EdgesCount();
        }

        if ((*data_graphs_i)[i]->MaxDegree() > max_degree_) {
            max_degree_ = (*data_graphs_i)[i]->MaxDegree();
        }
    }
...
void FSC::InitializeQueryGraphResource(const Graphcfql *query_graph) {
    ...
    Utilitycfql::GetKCore(query_graph, query_vertices_cores_);
...
bool FSC::GraphMetadataFilter(const Graphcfql *query_graph, const Graphcfql *data_graph) {
...
bool FSC::Simulation(const Graphcfql *query_graph, const Graphcfql *data_graph) {
...
bool FSC::Verification(const Graphcfql* query_graph, const Graphcfql *data_graph) {
    timeval ordering_start = Utilitycfql::GetTime();
    ComputeDirectedWeightedGraph(query_graph, data_graph);
    GenerateVerificationOrder(query_graph);
    GenerateBigraphIndex(data_graph);
    timeval ordering_end = Utilitycfql::GetTime();

    temp_ordering_time_ = Utilitycfql::TimeDiffInMicroseconds(ordering_start, ordering_end);
...
void FSC::UpdateSequenceElement(const Graphcfql* query_graph, const int pivot, const int u, SequenceElement &element) {
    element.id_ = u;
...
void FSC::GenerateNLF(const Graphcfql* query_graph) {
...
bool FSC::IsValid(const int level, const int v, const Graphcfql* data_graph) {
...
void FSC::GenerateBFSTree(const Graphcfql *query_graph, const Graphcfql *data_graph) {
...
void FSC::SelectRootVertex(const Graphcfql *query_graph, const Graphcfql *data_graph) {
...
bool FSC::GenerateRootCandidate(const Graphcfql *data_graph) {
...
void FSC::GenerateBigraphIndex(const Graphcfql *data_graph) {
...
void FSC::GenerateVerificationOrder(const Graphcfql *query_graph) {
...
void FSC::SelectNextVertex(const Graphcfql *query_graph, const int idx, int &next_query_vertex, int &pivot) {
...
void FSC::UpdateBackwardValue(const Graphcfql *query_graph, const int u) {
    int count;
    const int* neighbors = query_graph->Neighbors(u, count);
...
void FSC::SelectStartVertex(const Graphcfql *query_graph) {
...
void FSC::ComputeDirectedWeightedGraph(const Graphcfql *query_graph, const Graphcfql* data_graph) {
    for (int i = 0; i < simulation_order_count_; ++i) {
        int query_vertex = i;
