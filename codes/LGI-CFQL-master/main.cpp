...
int main( int argc, char *argv[] ){
    ...
    vector<Graphcfql*>* data_graphs_i = new vector<Graphcfql*>();
	for ( int i = 0; i < graphrd.querys.size (); i++ ){
		...
		vector<int> verification_set_idx;
        if ( candidate_flag != 0 && candidate_set.size () != 0 ) {
			timeval t3 = Utilitycfql::GetTime ();
			
			for ( int m3 = 0; m3 < candidate_set.size (); m3++ ) {
				data_graphs_i->push_back ( (*data_graphs)[candidate_set[m3]] );
			}
            FSC fsc ( &graphDB, data_graphs_i);	
			verification_set_idx.resize ( data_graphs_i->size() );
            // Process the query graphs.
			verification_set_idx = fsc.Query((*query_graphs)[i], verification_set_idx, verification_count);
			for ( int m5 = 0; m5 < verification_count; m5++ ) {
//				cout << verification_set_idx[m2] << " ";              
				verification_set.push_back ( candidate_set[verification_set_idx[m5]] );
			}
			...
			for ( int m8 = 0; m8 < data_graphs->size (); m8++ ) {
				data_graphs_i->push_back ( (*data_graphs)[m8] );
			}
			FSC fsc ( &graphDB, data_graphs_i);
			verification_set.resize ( data_graphs_i->size() );
			verification_set = fsc.Query((*query_graphs)[i], verification_set, verification_count);
